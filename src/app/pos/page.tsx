...
function POSContent({ user }: { user: { id: string; name: string; email: string; role: string } }) {
    ...
    async function fetchSettings() {
      try {
        ...
        if (res.ok) {
          ...
          setSettings({
            vat_enabled: data.tax?.find((s: any) => s.key === 'vat_enabled')?.value || 'false',
            vat_percentage: data.tax?.find((s: any) => s.key === 'vat_percentage')?.value || '0',
            currency_symbol: data.general?.find((s: any) => s.key === 'currency_symbol')?.value || '$',
            pharmacy_name: data.general?.find((s: any) => s.key === 'pharmacy_name')?.value || 'TechPharm Pharmacy',
            receipt_footer: data.pos?.find((s: any) => s.key === 'receipt_footer')?.value || 'Thank you for your business!',
          })
        }
      } catch (error) {
  ...
  function addToCart(drug: Drug) {
    ...
    const itemVatEligible = vatEnabled && drug.vatEligible
    ...
    if (existing) {
      ...
      const newSubtotal = newQty * drug.price
      const newVatAmount = itemVatEligible ? Math.round(newSubtotal * vatPercentage) / 100 : 0
      setCart(cart.map(item =>
        item.id === drug.id
          ? { ...item, quantity: newQty, subtotal: newSubtotal, vatAmount: newVatAmount }
          : item
      ))
    } else {
      if (drug.stock < 1) {
      ...
      const subtotal = drug.price
      const vatAmount = itemVatEligible ? Math.round(subtotal * vatPercentage) / 100 : 0
      setCart([...cart, {
        ...drug,
        quantity: 1,
        discount: 0,
  ...
  function updateQuantity(id: string, delta: number) {
    setCart(cart.map(item => {
      ...
      const newSubtotal = newQty * item.price - item.discount
      const itemVatEligible = vatEnabled && item.vatEligible
      const newVatAmount = itemVatEligible ? Math.round(newSubtotal * vatPercentage) / 100 : 0
      return { ...item, quantity: newQty, subtotal: newSubtotal, vatAmount: newVatAmount }
    }))
  }
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-sm text-slate-500">
                                ...
                                {vatEnabled && drug.vatEligible && (
                                  <Badge variant="secondary" className="ml-2 text-xs bg-orange-100 text-orange-700">VAT</Badge>
                                )}
                              </p>
                            </div>
                          </div>
                ...
                <CardContent className="p-6">
                  ...
                  <div className="bg-white rounded-lg p-4 text-sm">
                    <div className="text-center border-b pb-2 mb-2">
                      <p className="font-bold">{settings?.pharmacy_name || 'TechPharm PMS'}</p>
                    ...
                    <div className="border-t pt-2 mt-2">
                      <div className="flex justify-between text-slate-600">
                        <span>Subtotal</span>
                        <span>${receipt.subtotal?.toFixed(2) || subtotal.toFixed(2)}</span>
                      </div>
                      {receipt.tax > 0 && (
                        <div className="flex justify-between text-slate-600">
                          <span>VAT</span>
                    ...
                    {settings?.receipt_footer && (
                      <div className="border-t pt-2 mt-2 text-center text-xs text-slate-500">
                        {settings.receipt_footer}
                      </div>
                    )}
                  </div>
                          ...
                          <div key={item.id} className="bg-slate-50 rounded-lg p-3">
                            ...
                            <div className="flex items-center justify-between">
                              ...
                              <div className="text-right">
                                ...
                                {item.vatAmount > 0 && (
                                  <p className="text-xs text-orange-600">+${item.vatAmount.toFixed(2)} VAT</p>
                                )}
                              </div>
                            </div>
                          </div>
